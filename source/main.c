...
// The game stores its current language as a libc++ std::string at app+0xa20
// (from MyApp::ChangeLanguage in Burger Shop 2's libSexyAndroid.so). Its own
// save of that setting goes through a Java (JNI) path we don't have, so instead
// we read the live value and persist it to our own file, then feed it back to
// NativeAppStart at the next boot.
#define LANG_FILE       "language.txt"
#define APP_LANG_OFFSET 0xa20
